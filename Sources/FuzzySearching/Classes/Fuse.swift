...
public class Fuse: @unchecked Sendable {
	...
	public init(
		...
		tokenize: Bool = false,
		objSortStrategy: ObjectsSortStrategy = .minimalScore,
		...
		self.qos = qos
		self.searchQueue = DispatchQueue(label: "fuzzysearching.search.queue", qos: qos, attributes: .concurrent)
	}
	...
	public func createPattern(from str: String) -> Pattern? {
		...
		return Pattern(text: isCaseSensitive ? str : str.lowercased())
	}
}
